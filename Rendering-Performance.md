...
状态：**已完成低风险内存复制改造；高风险命令流和 GPU 常驻缓存改造暂不实施**。

本阶段关注 CPU 侧每帧热路径中的大块数据复制、容器复制和高频堆分配。目标不是机械地消灭所有复制，而是
让不可变的大型负载使用共享所有权，让线程边界所需的 Frame 快照保持独立，并保留 CPU 到 GPU 上传等必要复制。

当前审计结果：

| 优先级 | 场景 | 当前行为 | 结论 |
| ---: | --- | --- | --- |
| 高 | Raster Pass 使用同帧保存纹理 | Prepared Draw 保存该 Draw 实际引用的 RDG Texture；Pass 只声明实际使用的资源 | 已消除每个 Pass 捕获完整帧内纹理映射的重复容器复制 |
| 中 | Slate Target 转换通用命令 | 每帧逐条克隆命令并修改 Target 坐标；编译几何仍为共享引用 | 不复制顶点和索引，但会产生与命令数成正比的小对象分配 |
| 中 | Filter 与 Shader Frame 快照 | 持久描述符为不可变共享对象，Frame 只收集使用中的共享引用 | 已消除 `FString`、渐变 stop 数组等描述符负载的逐帧深复制 |
| 中低 | Texture Frame 快照 | Pipeline 资源集合未变化时复用逻辑快照；Target 绑定表仍逐帧刷新 | 避免重复逻辑 Map 分配，同时保证重导入和 RHI 恢复后不会使用过期绑定 |
| 中低 | Geometry 准备 | 顶点及默认索引区间按本帧唯一编译几何追加一次 | 重复 Draw 复用索引范围；只有兼容相邻 Draw 为形成连续合批时才追加索引 |
| 低且非每帧 | 文档与样式文件载入 | Provider 返回的独占字节数组直接转移给 `FSharedBuffer` | 已消除从临时 `TArray<uint8>` 到共享文件缓冲的第二次 payload 复制 |
| 必要 | RDG Vertex/Index 上传 | 每帧把准备后的几何写入 RDG Upload Buffer | 动态几何进入 GPU 的必要传输，不能通过共享指针消除 |
| 必要且非每帧 | 持久纹理初始化 | 首次使用或 RHI 恢复时，把共享像素写入 RHI 初始化内存 | 不是稳态每帧复制；为 GPU 纹理创建和设备恢复所必需 |
| 必要且非每帧 | 编译几何接收 | Geometry 编译时取得前端顶点和索引的独立所有权 | 保证前端回调结束后数据仍然有效，不能保存临时 ArrayView |

已经完成：

- [x] 生成纹理和磁盘纹理使用共享的只读像素 Buffer，不在 Pipeline、Target Cache 和持久纹理之间复制整张图片。
- [x] 持久纹理仅在首次 GPU 上传或 RHI 恢复时读取 CPU 像素，不再把上传复制放进每个 Raster Draw。
- [x] 已编译几何由命令共享持有，逐条命令克隆不会深复制顶点和索引数组。
- [x] 文档和样式文件载入时直接把 provider 字节数组所有权转移给共享缓冲，不再复制一次完整文件。
- [x] 已区分必要上传、必要生命周期快照和可消除的重复容器复制，避免用裸引用换取不安全的表面优化。

#### 内存回收边界

减少复制不等于立即释放内存。只要 Render Thread 或 RDG Pass 仍可能读取一份数据，该数据就必须由在途 Frame
继续持有；安全回收点是最后一个消费者结束，而不是 Game Thread 完成提交。

| 内存类别 | 安全回收时机 | 是否可以提前回收 |
| --- | --- | --- |
| Raster Pass 捕获的临时容器 | 对应 RDG Graph 执行完成后 | 可以消除重复容器，但 Pass 实际使用的 RDG 资源引用必须保留到 Graph 完成 |
| Slate Target 的转换后命令 | Custom Draw Element 在 Render Thread 消费完成后 | 只有源命令和 Target 转换状态均由在途 Frame 强持有时，才能取消命令克隆 |
| Texture、Filter、Shader 快照 | 最后一个引用该版本的在途 Frame 消费完成后 | 可以通过引用计数自动回收旧版本，不能在资源集合更新时原地修改或立即销毁旧版本 |
| 每帧 Prepared Draw、Vertex 和 Index CPU 数组 | RDG 已取得上传数据且 Pass 建立完成后 | 不能让 RDG 延迟上传引用函数栈或下一帧会修改的数组；使用 NoCopy 上传前必须另行证明生命周期 |
| RHI 纹理初始化 staging 数据 | RHI 纹理初始化完成后 | 可以由 RHI 自动回收，不应由业务代码保存第二份 staging 副本 |
| 纹理 CPU 像素 backing | 对应逻辑纹理已释放，且所有在途 Frame、Target Cache 和 RHI 恢复监听者均不再持有时 | 当前不能在首次 GPU 上传后统一释放；生成纹理和字体 atlas 未必能从外部资源重新构造 |
| 编译几何 CPU 数据 | Frontend 释放 Geometry，且引用它的所有在途 Frame 已消费后 | 不能只因本帧上传完成而释放；后续帧仍可能重复绘制同一 Geometry |
| 持久 GPU 纹理或保存的 Layer | Frontend 释放对应 handle，且最后一个在途 Frame 已消费后 | 不能根据当前页面暂时没有 Draw 就提前释放，否则跨帧纹理和资源恢复会失效 |

`FSharedBuffer`、共享资源和不可变快照的正确回收方式是移除拥有者并等待最后一个强引用自然释放，而不是
主动清空仍可能被 Render Thread 使用的内存。短时间内同时存在当前版本和旧版本属于有界的 frames-in-flight
成本，不是泄漏。

#### 按风险和收益排序的实施任务

| 顺序 | 任务 | 能否安全减少或回收内存 | 实施结论 |
| ---: | --- | --- | --- |
| 1 | [x] Raster Pass 只保存实际引用的同帧纹理，不再捕获完整帧内纹理映射 | **可以**。删除的是每个 Pass 重复生成的 Map 节点；实际 RDG Texture 引用保留到 Graph 完成 | Prepared Draw 保存已经解析的 RDG Texture，Pass Parameters 只声明实际使用的资源 |
| 2 | [x] 为 Pipeline Texture 集合增加 dirty 快照复用 | **可以**。集合不变时不产生新容器；旧版本在最后一个在途 Frame 完成后自动回收 | 已发布快照保持不可变，资源集合变化时创建新版本。Target 绑定仍逐帧刷新，避免缓存重导入或 RHI 恢复后的过期引用 |
| 3 | [x] 减少 Filter 和 Shader Frame 描述符复制 | **可以**。持久描述符以不可变共享对象保存，Frame 只复制被引用对象的共享指针 | 未采用简单 Map `MoveTemp`；在途 Frame 强持有其使用的描述符版本 |
| 4 | [ ] 统计命令克隆数量、Snapshot 重建数量、条目数和 CPU 准备时间，再评估 Slate 命令 Clone | **有条件可以**。转换后命令可在 Render Thread 消费后回收，但取消克隆后仍须强持有源命令与 Target 状态 | 先统计。若确认是热点，优先改为连续数组或不可变源命令加 Target 转换上下文 |
| 5 | [x] 非合批 Draw 复用已追加的 Index Range，仅在合批确实要求连续索引时复制 | **可以**。本帧 Index Buffer 可以缩小，原有编译几何继续由共享引用持有 | 重复几何默认复用索引区间；只有兼容的相邻 Draw 为保持连续合批才追加索引副本 |
| 6 | [ ] 评估把稳定编译几何缓存为持久 GPU Buffer | **通常不能降低总内存**。它减少逐帧上传，却增加 GPU 常驻资源；为 RHI 恢复通常仍需 CPU 数据 | 当前不实施，除非 Insights 证明动态上传是实际 CPU 或总线瓶颈 |

额外结论：磁盘纹理理论上可以在首次上传后丢弃解码像素，并在 RHI 恢复时重新读取和解码文件，但这会把
文件 I/O、文件变更、资源丢失和恢复线程时序引入设备恢复路径。生成纹理和字体 atlas 又不具备等价的磁盘源，
因此当前不采用按来源分裂的提前回收策略；统一保留一份只读 CPU backing，直到逻辑资源真正释放。

安全约束：

1. 不允许 Render Thread Pass 引用 Game Thread 上仍会变化的 Map、Array 或命令对象。
2. 不允许用裸指针或弱引用替代已提交 Frame 对像素、几何、纹理和效果描述符的强引用。
3. `FSharedBuffer`、`TSharedPtr` 和 `TSharedRef` 的值传递只增加引用计数，不应被误判为负载深复制。
4. 小型矩形、矩阵、设置结构和句柄应继续按值保存；为几十字节状态引入额外共享分配通常是负优化。
5. 每项优化必须分别比较 CPU Frame/RenderThread 时间、分配次数和上传字节，并在最后集中执行自动化测试和
   Viewer 视觉回归。

本阶段已完成前三项低风险热路径改造和索引区间复用。Slate Target 仍需为坐标转换克隆命令；取消该克隆
会改变当前的不可变命令流和跨线程所有权边界，因此在取得 CPU 分配与耗时证据前不继续推进。
...
| 10 | 每帧内存复制与分配优化 | 低风险热路径改造已完成 | 降低 RenderThread 容器复制、描述符深复制和重复索引上传 | 中，必须保持在途 Frame 独立性 |
...
| 2 | Slate 转换命令克隆评估 | 尚未实现；先用 Insights 或分配统计证明它是热点，不以可变共享命令破坏在途 Frame 独立性 |
| 3 | 扩大几何合批兼容范围 | 当前决定不实现；现有约 `0.08%` 命中率不足以证明架构改造收益 |
| 4 | 更精确的 Transform Bounds | 当前决定不实现；只有 Bounds 日志证明异常完整视口扩张后重新开启 |
| 5 | 同 Layer 局部混合采样 | 当前决定不实现；需证明收益能够覆盖新增资源和 Composite 成本 |
| 6 | 默认路径的状态与 Pass 优化 | 尚无具体无损方案；等待 GPU 工具定位真实热点 |
...
任务 10 已完成像素与几何负载的共享所有权改造、每帧复制审计、Raster Pass 实际纹理引用解析、Pipeline
纹理快照复用、Filter/Shader 不可变描述符共享和重复几何索引区间复用。Slate 转换命令克隆与持久 GPU
Geometry 未改：前者等待 CPU 热点证据，后者可能增加 GPU 常驻内存，均不属于本轮可证明收益的安全改造。

已完成的本地验证：

- Development Editor 构建通过。
- MarkupUI 完整 C++ Automation Test 套件 72/72 通过，覆盖 Layer 保存与生命周期、Mask、Backdrop、Blur、
  Drop Shadow、同 Layer scratch、几何合批、共享缓冲所有权、逻辑纹理快照复用及可选矩阵融合。
