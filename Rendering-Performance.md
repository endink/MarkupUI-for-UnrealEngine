...
状态：**已完成内存复制审计与像素负载共享改造，其余优化尚未实施**。
...
| 优先级 | 场景 | 当前行为 | 结论 |
| ---: | --- | --- | --- |
| 高 | Raster Pass 使用同帧保存纹理 | 每个 Raster Pass 捕获一份完整的帧内纹理映射 | Pass 数量较多时会重复分配和复制容器；应优先消除 |
| 中 | Slate Target 转换通用命令 | 每帧逐条克隆命令并修改 Target 坐标；编译几何仍为共享引用 | 不复制顶点和索引，但会产生与命令数成正比的小对象分配 |
| 中 | Filter 与 Shader Frame 快照 | 使用中的描述符会在 Pipeline、Target 和 Target Cache 之间形成多次容器复制 | 可通过移动所有权和不可变快照减少，必须保留在途 Frame 生命周期 |
| 中低 | Texture Frame 快照 | 每帧重建逻辑纹理表和 Target 绑定表 | 不复制像素，只复制 Map 节点、轻量绑定和共享引用；适合版本化复用 |
| 中低 | Geometry 准备 | 顶点按本帧唯一编译几何追加一次，索引仍按 Draw 实例追加 | 顶点复用已生效；索引可进一步复用，但预期收益受几何复用率限制 |
...
- [ ] 在建立 Raster Pass 时，只解析并保存该 Pass 实际引用的同帧纹理；Prepared Draw 直接持有对应的
  RDG Texture 引用，不再让每个 Pass 捕获完整帧内纹理映射。
- [ ] Frame 提交时把当帧 Filter 和 Shader 描述符移动到不可变快照，消除从临时 Frame Map 到 Snapshot 的
  一次完整复制。
- [ ] 为 Texture、Filter 和 Shader 资源集合增加 revision/dirty 机制；集合未变化时复用不可变快照，变化时
  创建新快照，旧快照继续由已经提交的 Frame 强引用持有。
- [ ] 增加 CPU 侧准备时间、命令克隆数量、Snapshot 重建数量和 Snapshot 条目数统计，再决定是否重构 Slate
  Target 的逐命令 Clone。没有测量证据前，不用可变共享命令替换当前安全的不可变命令流。
- [ ] 在几何复用率和 `indexUploadBytes` 证明收益足够时，让非合批 Draw 复用已追加的 Index Range；只有确实
  需要连续索引区间的相邻合批才追加索引副本。
- [ ] 最后才评估将稳定编译几何缓存为持久 GPU Buffer。该方案会增加 GPU 常驻内存、资源失效恢复和碎片管理
  成本，必须由 Unreal Insights 或 RHI 统计证明动态上传是实际瓶颈后才能实施。
...
本阶段的第一实施项应是移除 Raster Pass 对完整帧内纹理映射的复制。它不会改变命令顺序、像素内容、
Filter 语义或跨线程资源所有权，风险显著低于命令流结构重构。
...
| 10 | 每帧内存复制与分配优化 | 审计及像素负载共享已完成 | 降低 RenderThread 容器复制、分配和 CPU 上传准备成本 | 中，必须保持在途 Frame 独立性 |

未完成工作的当前优先级：

| 优先级 | 未完成工作 | 当前决定 |
| ---: | --- | --- |
| 1 | 消除 Raster Pass 的完整帧内纹理映射复制 | 尚未实现；属于当前内存阶段风险最低的首项工作 |
| 2 | Frame 描述符移动与资源快照版本化 | 尚未实现；需保持已经提交 Frame 的强引用和不可变语义 |
| 3 | 同帧 Snapshot 复用 | 尚未实现；先增加命中与避免像素统计 |
| 4 | 扩大几何合批兼容范围 | 当前决定不实现；现有约 `0.08%` 命中率不足以证明架构改造收益 |
| 5 | 更精确的 Transform Bounds | 当前决定不实现；只有 Bounds 日志证明异常完整视口扩张后重新开启 |
| 6 | 同 Layer 局部混合采样 | 当前决定不实现；需证明收益能够覆盖新增资源和 Composite 成本 |
| 7 | 默认路径的状态与 Pass 优化 | 尚无具体无损方案；等待 GPU 工具定位真实热点 |
...
任务 10 已完成像素与几何负载的共享所有权改造和每帧复制审计；Raster Pass 帧内纹理映射、Frame 描述符
移动、资源快照版本化、Slate 命令分配及索引范围复用仍是未完成任务，不能描述为已优化完成。
